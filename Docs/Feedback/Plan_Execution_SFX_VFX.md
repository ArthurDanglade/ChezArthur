...
### F2-P2 — Re-câblage groupe A + hit-react allié (VALIDÉE — Go du 03/08, 3 questions : oui / oui / oui ; prompt P2a livré)
...
| 03/08 | F2-P2a — Go + prompt | — (HEAD `f71efb6`) | Go acté (P2a→P2b · plafonds A dès P2a · hit-react 5/100 ms/neutre). Relecture à HEAD : JuiceDirector intact depuis `ba7768f`, Float/Factory intacts depuis G1-P1, shader `ChezArthur/SpriteFlash` multiplie la couleur vertex (swap allié sûr). Prompt livré — design clé : API statiques à **repli legacy** (commit code sans risque avant commit scène), squash allié composé DANS CharacterBallFloat (état flinch, pas de double écrivain sur le Visual). Attendu : 2 commits (code / scène). |
